...
document.addEventListener('DOMContentLoaded', () => {
      ...
      return `
        <div class="success-overlay" role="status" aria-live="polite">
          <div class="success-content">
            ...
            <button class="btn-continue" id="btn-continue-login">Continuar</button>
          </div>
        </div>
      `
    }
    return `
      <div class="success-overlay-reg">
        <div class="success-content-reg">
          <div class="success-crown-reg">👑</div>
          <h2>Conta criada!</h2>
          <p>Bem-vinda à Liz, ${userName}</p>
          <button class="btn-continue-reg" id="btn-continue-register">Ir para o login</button>
        </div>
      </div>
    `
  }
    ...
    return `
      <div class="success-overlay-reg">
        <div class="success-content-reg">
          ...
          <button class="btn-continue-reg" id="btn-back-login">Voltar ao login</button>
        </div>
      </div>
    `
  }
  ...
  function bindEvents() {
    ...
    if (btnContinueLogin) {
      btnContinueLogin.addEventListener('click', () => {
        const overlay = btnContinueLogin.closest('.success-overlay')
  ...
  async function handleLoginSubmit(e) {
    ...
    if (btnContinue) {
      btnContinue.addEventListener('click', () => {
        const overlay = btnContinue.closest('.success-overlay')
        if (overlay && overlay.parentNode) overlay.parentNode.removeChild(overlay)
        document.getElementById('login-form').reset()
        // Reset field states
  ...
  async function handleRegisterSubmit(e) {
    ...
    const displayName = nameInput.value.split(' ')[0]
